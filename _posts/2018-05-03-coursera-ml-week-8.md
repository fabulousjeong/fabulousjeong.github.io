...
- projection error의 제곱 평균이 주어짐: $\dfrac{1}{m}\sum^m_{i=1}||x^{(i)} - x_{approx}^{(i)}||^2$
- 전체 데이터의 variation을 구함: $\dfrac{1}{m}\sum^m_{i=1}||x^{(i)}||^2$
- 다음 값을 만족하는 k를 선택: $\dfrac{\dfrac{1}{m}\sum^m_{i=1}||x^{(i)} - x_{approx}^{(i)}||^2}{\dfrac{1}{m}\sum^m_{i=1}||x^{(i)}||^2} \leq 0.01$
